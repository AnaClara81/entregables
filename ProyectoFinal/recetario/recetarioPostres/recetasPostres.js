...
const getRecetas = async () =>{
    let response = await fetch('./recetasPostres.json');
    let data = await response.json();
    let recetasPostres = data
 

    recetasPostres.forEach(receta => {
  ...
  function guardarStorage(){
   localStorage.setItem("receta",JSON.stringify(recetasPostres))
}
