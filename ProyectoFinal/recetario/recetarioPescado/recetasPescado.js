...
const getRecetas = async () =>{
    let response = await fetch('./recetasPescado.json');
    let data = await response.json();
    let recetasPescado = data
 

    recetasPescado.forEach(receta => {
  ...
  function guardarStorage(){
   localStorage.setItem("receta",JSON.stringify(recetasPescado))
}
