...
const getRecetas = async () =>{
    let response = await fetch('./recetasVerduras.json');
    let data = await response.json();
    let recetasVerduras = data
 

    recetasVerduras.forEach(receta => {
  ...
  function guardarStorage(){
   localStorage.setItem("receta",JSON.stringify(recetasVerduras))
}
